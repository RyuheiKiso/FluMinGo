...
## 目次
1. [Flutter プロジェクト概要](#1-Flutter-プロジェクト概要)
2. [golang プロジェクト概要](#2-golang-プロジェクト概要)
3. [補足 (高度な設計観点)](#3-補足-高度な設計観点)
...
- **di**：依存性注入の設定と管理
- **screens**：各画面のUIとロジックの管理
- **util**：ユーティリティ関数や共通コンポーネントの管理
- **common_api**：共通機能の実装と管理
- **specific_api**：個別APIの実装と管理
- **unit_tests**：ユニットテストの実装と管理

各ディレクトリの詳細については、各ファイルのコメントを参照してください。

## 5. 各ディレクトリの詳細な説明
- **main.dart**：アプリケーションのエントリーポイント。アプリ全体の初期化とルーティングを担当。
- **di**：依存性注入の設定と管理。サービスやリポジトリのインスタンスを管理し、各モジュールに提供。
- **screens**：各画面のUIとロジックの管理。画面ごとにMVC/MVVM/BLoCパターンを実装し、内部ロジックとUIを分離。
- **util**：ユーティリティ関数や共通コンポーネントの管理。再利用可能なコードを集約し、コードの重複を防止。
- **common_api**：共通機能の実装と管理。認証、DB接続、ロギングなどの基盤機能を提供。
- **specific_api**：個別APIの実装と管理。各APIの仕様、ビジネスロジック、データモデルを分離。
- **unit_tests**：ユニットテストの実装と管理。各モジュールのユニットテストを配置し、テストカバレッジを向上。
